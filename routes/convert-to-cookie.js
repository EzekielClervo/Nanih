module.exports = (fetch) => async function convertToCookie(req, res) {
    try {
        const { username, password } = req.body;
        const cookie = await getCookie(username, password, fetch);
        res.json(cookie);
    } catch (error) {
...
async function getCookie(username, password, fetch) {
    const headers = {
